...
{var options=$.extend({},$.fn.formset.defaults,opts);var updateElementIndex=function(el,prefix,ndx){var id_regex=new RegExp('('+prefix+'-\\d+)');var replacement=prefix+'-'+ndx;if($(el).attr("for"))$(el).attr("for",$(el).attr("for").replace(id_regex,replacement));if(el.id)el.id=el.id.replace(id_regex,replacement);if(el.name)el.name=el.name.replace(id_regex,replacement);};$(this).each(function(i){$(this).addClass(options.formCssClass);if($(this).is('TR')){$(this).children(':last').append('<a class=" alonso ' +options.deleteCssClass+'" href="javascript:void(0)">'+options.deleteText+'</a>');}else if($(this).is('UL')||$(this).is('OL')){$(this).append('<li><a class="alonso '+options.deleteCssClass+'" href="javascript:void(0)">'+options.deleteText+'</a></li>');}else{$(this).children('div:last').children(1).append('<a class="col-sm-1 btn btn-danger  '+options.deleteCssClass+'" href="javascript:void(0)">'+options.deleteText+'</a>');}
...
return $(this);}
$.fn.formset.defaults={prefix:'form',addText:'Añadir más',deleteText:'-',addCssClass:'add-row',deleteCssClass:'delete-row',formCssClass:'dynamic-form',added:null,removed:null}})(jQuery)
